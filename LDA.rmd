Setup
```{r}
heart <- read.table("heart_disease3.txt")
set.seed(1234)
...
LDA Assumptions
```{r}
assump <- na.omit(read.table("heart_disease3.txt"))

library(heplots)
boxm <- heplots::boxM(assump[,2:11], assump$HeartDiseaseorAttack)
boxm
plot(boxm)


heart.yes <- subset(assump, HeartDiseaseorAttack == 1)
heart.no <- subset(assump, HeartDiseaseorAttack == 0)
